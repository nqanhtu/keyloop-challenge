...
export const inventoryHandlers = [
  http.get('/vehicles', async ({ request }) => {
    ...
    const result = await defaultInventoryService.queryVehicles({
      make,
      model,
      ageMinDays,
      ageMaxDays,
  ...
  http.get('/vehicles/:vehicleId', async ({ params }) => {
    ...
    const vehicle = await defaultInventoryService.getVehicleById(vehicleId);
  ...
  http.get('/inventory/filter-options', async ({ request }) => {
    const url = new URL(request.url);
    const make = url.searchParams.get('make') ?? undefined;
    const options = await defaultInventoryService.getFilterOptions(make);
